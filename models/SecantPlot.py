    ...
    def update_figure(self):
        ...
        try:
            ...
            if self.Fxbl[i] * self.Fxl[i] < 0:
                self.axes.plot([self.xl[i], self.xbl[i]], [self.Fxl[i], self.Fxbl[i]], color='g', lw=1, linestyle='dashed')
            else:
                if abs(self.Fxl[i]) > abs(self.Fxbl[i]):
                    self.axes.plot([self.xl[i], self.xr[i]], [self.Fxl[i], 0], color='g', lw=1, linestyle='dashed')
                else:
                    self.axes.plot( self.xr[i],[self.xbl[i]], [0,self.Fxbl[i]], color='g', lw=1, linestyle='dashed')
        ...
        if self.Fxbl[i]*self.Fxl[i] < 0:
            ...
            if abs(self.Fxl[i]) > abs(self.Fxbl[i]):
                ...
                self.animateLine( self.xr[i],self.xbl[i], 0, self.Fxbl[i], self.xr[i], self.xs, self.ys)
