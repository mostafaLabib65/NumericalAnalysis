...
class FalsePositionPlot(PlotFigure):
    def __init__(self,xs, ys, xr, xl,Fxl, xu,Fxu, app=None, parent=None, width=5, height=4, dpi=100):
    ...
    def compute_initial_figure(self):
        self.axes.axvline(x=0, lw=2, color='k', label='axes')
        self.axes.axhline(y=0, lw=2, color='k')
        self.axes.set_xlabel('x')
        self.axes.set_ylabel('y')
        self.axes.set_title('False position method')
    ...
    def update_figure(self):
        ...
        try:
            ...
            self.axes.plot([self.xl[len(self.xr) - 1], self.xu[len(self.xr) - 1]], [self.Fxl[len(self.xr) - 1], self.Fxu[len(self.xr) - 1]], color='g', lw=1,
                     linestyle='dashed')
            return
        self.animateLine(self.xl[i], self.xu[i], self.Fxl[i], self.Fxu[i], self.xr[i], self.xs, self.ys)
